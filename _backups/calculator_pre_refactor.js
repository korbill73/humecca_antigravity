...
document.addEventListener('DOMContentLoaded', () => {
    initCalculator();
    // updateSummary(); // Moved inside init
});

async function initCalculator() {
    // Show Loading
    const list = document.getElementById('product-list');
    if (list) list.innerHTML = '<div style="color:white; text-align:center; padding:50px;">데이터를 불러오는 중...</div>';

    // Fetch Data
    if (typeof fetchCalculatorData === 'function') {
        await fetchCalculatorData();
    } else {
        console.error('fetchCalculatorData is not defined');
    }
